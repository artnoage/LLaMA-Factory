...
def generate_grid(name, existing_grids, min_width=3, max_width=20, min_height=3, max_height=20):
    ...
    if existing_grids and random.random() < 0.5:  # 50% chance if there are existing grids
        ...
        max_attempts = 5  # Maximum number of attempts to apply a relationship
        for _ in range(max_attempts):
            relationship = get_random_relationship()
            new_grid = relationship.apply(np.copy(reference_grid['array']))
            
            if isinstance(new_grid, str) and new_grid == "reject":
                continue  # Try another relationship
            
            # Check if the new grid is equal to the reference grid
            if isinstance(new_grid, np.ndarray) and np.array_equal(new_grid, reference_grid['array']):
                tag = "random grid"
                arg_info = {}
            elif isinstance(new_grid, np.ndarray):
                tag = f"generated by grid {reference_grid['name']} using the following relationship: {relationship.get_description()}"
                ...
                continue  # If new_grid is neither a string nor an array, try another relationship
        
        # If all attempts failed, create a random grid
        tag = "random grid"
        arg_info = {}
    else:
        tag = "random grid"
